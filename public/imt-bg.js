(() => {
  ...
  function ensureHistoryPlaceholder() {
    ...
    if (!document.getElementById('imt-github-button')) {
      const newChatButton = sidebar.querySelector('button[data-testid*="new" i], button[aria-label*="new" i], header button');
      
      if (newChatButton) {
        console.log('Bouton Nouvelle discussion trouvé:', newChatButton);
        console.log('Classes du bouton:', newChatButton.className);
        ...
        githubBtn.className = bhhffffggggghhhhhhuttonClasses;
        githubBtn.innerHTML = `
        ...
        newChatButton.parentElement.insertBefore(githubBtn, newChatButton.nextSibling);
        newChatButton.parentElement.insertBefore(langfuseBtn, githubBtn.nextSibling);
        
        console.log('Boutons GitHub et Langfuse ajoutés avec succès');
      } else {
        console.log('Bouton Nouvelle discussion non trouvé');
      }
    }
